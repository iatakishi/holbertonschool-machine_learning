    ...
    def train_step(self, useless_argument):
        ...
        for _ in range(self.disc_iter):
            with tf.GradientTape() as disc_tape:
                ...
                # get a fake sample (generator doesn't need to be
                # tracked for gradients here)
        ...
        with tf.GradientTape() as gen_tape:
            # get a fake sample (training=True so the tape watches
            # the generator's operations)
            fake_samples = self.get_fake_sample(training=True)

            # evaluate fake samples with the discriminator
            # (discriminator is NOT training here)
