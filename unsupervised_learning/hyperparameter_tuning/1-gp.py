...
class GaussianProcess:
    ...
    def predict(self, X_s):
        ...
        # Calculate the inverse of the initial covariance matrix
        K_inv = np.linalg.inv(self.K)

        # Calculate the posterior mean
        mu = K_s.T.dot(K_inv).dot(self.Y)
        mu = np.squeeze(mu)  # Reshapes from (s, 1) to (s,)

        # Calculate the posterior covariance matrix
        cov_s = K_ss - K_s.T.dot(K_inv).dot(K_s)

        # The variance for each point is the diagonal of the covariance matrix
        sigma = np.diag(cov_s)
